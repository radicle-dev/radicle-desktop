...
| `valid-members-actors.json` | Members given entirely as actor RIDs. |
| `invalid-member-mixed-form.json` | A bare RID missing its prefix, and a `rad://` URL, among members. |
...
- A `members` entry is either an actor repository (`rad:`, the recommended
  form for people, since it survives device changes and can be attested) or a
  bare device key (`did:key:`, a claim-only roster entry). The app accepts
  both; it does not yet resolve the actor form, which needs the Actor Repos
  RIP in heartwood.
