<script lang="ts">
  ...
  useShortcuts(
    ...
    },
    {
      ...
      run: () => {
        ...
        if (!modalHasTextEntry()) {
          // Matches the sidebar's reload button rather than re-fetching per
          // route.
    ...
    },
    {
      shortcut: "settings",
      allowInModal: true,
      run: () => toggle({ component: SettingsView, props: {} }),
    },
    {
...
<svelte:document onmousedown={dragWindow} />
