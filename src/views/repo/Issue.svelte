  ...
  <div class="content">
      ...
      <CommentComponent
        ...
        <svelte:fragment slot="actions">
          <Icon
            styleCursor="pointer"
            name="reply"
            onclick={() => (topLevelReplyOpen = !topLevelReplyOpen)} />
        </svelte:fragment>
    ...
    <div>
      {#await invoke<IssueOps[]>( "activity_by_id", { rid: repo.rid, typeName: "xyz.radicle.issue", id: issue.id }, ) then activity}
        {#each activity.slice(1) as issueOp}
          {#if issueOp.action.type === "lifecycle"}
            <IssueTimelineLifecycleAction {issueOp} />
            <div class="connector"></div>
          {:else if issueOp.action.type === "comment"}
            {@const thread = threads.find(t => t.root.id === issueOp.id)}
            {#if thread}
              <Thread
                {thread}
                rid={repo.rid}
    ...
    <CommentToggleInput
      ...
      rid={repo.rid}
      focus
      onexpand={() => (topLevelReplyOpen = true)}
