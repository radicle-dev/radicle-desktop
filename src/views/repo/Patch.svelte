<script lang="ts">
  ...
  // Only delegates and the patch author can change patch state or edit the
  // title.
  ...
  // Scroll the timeline down to the newest activity. The page scrolls inside
  // the OverlayScrollbars viewport (not the window), so find that viewport from
  // the bottom sentinel and scroll it to the end; the newest revision/comment
  // and the reply box sit there.
  ...
  // Only offer "Jump to most recent" when the bottom of the page is actually
  // off-screen. A sentinel at the end of the scroll area drives this: on a
  // short patch (or once you've scrolled down) the bottom is visible, so the
  // button hides itself.
  ...
  // Which sets of code comments are superimposed on the diff. Every source is
  // shown until hidden, and the set is cleared when the revision changes since
  // the ids belong to that revision's reviews.
  ...
  // Both columns are supplementary, so they can be folded away when the
  // dropdown only needs to answer "which revision is which".
  ...
  // The review column has to be one width for all rows: the columns to its left
  // are right-aligned as a group, and a per-row width would drag them out of
  // line. Rather than adding up per-glyph constants — which drift the moment an
  // icon, avatar size or font changes — an off-screen copy of the widest cell
  // is rendered and measured.
  //
  // The widest cell is the row with the most reviewers, plus the draft marker if
  // any row carries one. That combination need not exist in the list, but it is
  // never narrower than one that does, so the column always fits.
                          ...
                          <div
                            ...
                            <div class="revision-sort">
                              ...
                              <span
                                ...
                                <!-- A plain menu rather than a Popover: only
                                     one Popover can be open at a time, so a
                                     nested one would close the dropdown. -->
                                {#if columnMenuOpen}
