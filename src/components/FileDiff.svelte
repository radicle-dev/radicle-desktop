<script lang="ts">
  ...
  interface Props {
    ...
    // Set when the changeset is virtualized: the block's own sticky header
    // can't work inside an absolutely-positioned row, so the list draws a
    // pinned header itself and owns the collapse state.
    sticky?: boolean;
    headerOnly?: boolean;
  ...
  const filePathKey = $derived(fileDiffPath(file));
  ...
  // Only review comments can be resolved. Draft and standalone revision
  // comments carry a `resolved` flag that no action can ever set, so counting
  // them would leave the file permanently showing unresolved work.
  const resolvableThreads = $derived(
    commentsOfThisFile
      ? commentsOfThisFile.threads.filter(
          t => commentsOfThisFile.canResolveComment?.(t.root.id) ?? true,
        )
      : [],
  );
  const resolvedThreads = $derived(
    resolvableThreads.filter(t => t.root.resolved === true).length,
  );
  const unresolvedThreads = $derived(
    resolvableThreads.filter(t => t.root.resolved === false).length,
  );
</script>
    ...
    {#if draftReviewId}
      <Button
        ...
        onclick={e => {
          ...
          // Collapse when marking reviewed, re-expand when un-marking. Report
          // it too, so an owner tracking collapse state stays in step.
