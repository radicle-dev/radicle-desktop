<script lang="ts" module>
  ...
  // Map Pierre's light/dark theme pair to bundled Shiki themes. Token-exact
  // mapping onto the app's `--color-prettylights-syntax-*` design tokens (via a
  // CSS-variables theme) is the theming follow-up; bundled themes get the spike
  // rendering first.
...
<script lang="ts">
  ...
  interface Props {
    // Unified `git diff` patch text, as returned by the `get_diff_text`
    // backend command (`getDiffText` in `@app/lib/invoke`).
    patch: string;
    diffStyle?: "unified" | "split";
    // Diff view preferences (see `diffOptions.svelte.ts`). Mapped to Pierre's
    // options in `options()` below.
  ...
  // expand markers — replacing our old load-then-swap header button. Stable
  // reference so option updates never re-trigger loader wiring. Pierre only
  // invokes it for changed/renamed files (added/deleted already carry a side).
  ...
  function options(
    ...
    return {
      ...
      themeType,
      diffStyle: style,
      // Diff view preferences from `diffOptions` (passed as props).
      disableBackground,
      disableLineNumbers,
      ...
      // Our `DiffFileHeader` is a fixed-height row, so give Pierre the exact
      // header height up front. Setting it statically (vs. measuring at runtime
      // and calling `setOptions` afterwards) avoids an `itemMetrics`-change
      // layout reset that races with in-flight highlight results — that race
      // intermittently left files rendering against a stale highlight
      // ("deletionLine and additionLine are null"). `lineHeight` must equal the
      // rendered row height (`--diffs-line-height`, set to the same
      // `lineHeightPx`); Pierre's default is a fixed 20px, which only matches at
      // the 16px root font — at other font sizes the body height (and so
      // scroll-to-file) would drift.
      itemMetrics: {
