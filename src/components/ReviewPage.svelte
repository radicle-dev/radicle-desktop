<script lang="ts">
  ...
  // Comments on the review itself rather than on a line. A review's summary is
  // a field of its own, not the root of this thread, so nothing here has to be
  // filtered out for it; a comment whose `replyTo` is the review id is a reply
  // to that summary and starts a root thread of its own, since the summary is
  // not rendered as one.
