...
<script lang="ts">
  ...
  interface Props {
    // Flat list of file paths to show (e.g. the files changed in a commit).
    paths: string[];
    // Optional per-path git status, used for the tree's status indicators.
    gitStatus?: GitStatusEntry[];
    // Called with a file path when a file (not a folder) is selected.
