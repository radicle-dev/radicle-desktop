<script lang="ts">
  ...
  // Group a flat list of code threads by file path so each file renders one
  // header with its comment hunks stacked beneath, rather than a duplicate
  // file block per comment.
  ...
  // Every code-comment mutation dispatches on which object owns the comment,
  // so resolving that once here keeps the actions themselves shared.
