<script lang="ts">
  ...
  // No fallback: `draftReviewStorage.create` records the starting verdict, so
  // an absent one here means the reviewer chose "Comment". Defaulting at this
  // point would both undo that choice and write "accept" back to the draft the
  // next time the summary is saved.
  // svelte-ignore state_referenced_locally
  ...
  async function publish() {
    ...
    try {
      ...
      // everything typed so far is still here to retry with. Say so, rather
      // than letting the failure surface as an unhandled rejection and leaving
      // the bar looking like nothing happened.
