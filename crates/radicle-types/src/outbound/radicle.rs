...
impl RepoStorage for Radicle {
    fn create_repo(
        ...
        let base_commit = project.backend.find_commit(base)?;
        project
            .backend
            .branch(default_branch.as_str(), &base_commit, true)?;
        project.backend.set_head(branch(&default_branch).as_str())?;
