...
pub trait PatchesMut: Profile {
    ...
    /// ignores a second `Review` action, so callers must not offer this where a
    ...
    ///
    /// Only the COB ref under our own namespace is dropped; a patch replicated
    /// from another peer stays in their storage. On a patch we did not author we
    /// hold no such ref, so `Store::remove` finds nothing to delete and returns
    /// `Ok` — while the cache entry is still evicted, hiding the patch locally
    /// until the next cache rebuild. Callers must therefore restrict this to
    /// patches authored by the local node.
    fn delete_patch(
        &self,
        rid: identity::RepoId,
        cob_id: git::Oid,
