...
pub fn list_notifications(
    ...
    // Get total notification counts for each repository
    ...
    // When all is true, explicitly ignore the take parameter
    let take = if params.all.unwrap_or(false) {
        usize::MAX // Use maximum value when all is true, ignoring any take parameter
    } else {
        params.take.unwrap_or(20)
    };

    // Group notifications by repository ID to avoid duplicates
    ...
    for (repo_id, all) in grouped_repos {
        ...
        // Get the patch and issue caches
        ...
        // Only add repositories with notifications to the result
        if !content.is_empty() {
            // Get the total count for this repository, defaulting to the displayed notifications count if not available
